...
void MA_Write_Material (unsigned int m, MA_EXPORT &MA)		// DA SISTEMARE
{
	...
	case (Material::TYPE::LAMBERT):
		out << "createNode lambert -n \"" << mat.name << "\";\n";
		...
		{
			if (mat.Blend == Material::BLEND::SUBTRACT)							// Blend Subtract
			{
				...
				out << "connectAttr \"" << mat.name << "colorComposite.msg\" \":defaultRenderUtilityList.u\" -na;\n";
		...
		if (!mat.bump.empty())												// Bump map
		{
			...
			out << "connectAttr \"" << mat.name << "bump2d.msg\" \":defaultRenderUtilityList.u\" -na;\n";
		}
		if (!mat.transparency.empty())										// Trasparenza
			out << "connectAttr \"" << mat.transparency << ".ot\" \"" << mat.name << ".it\";\n";
		if (mat.Blend == Material::BLEND::OVERLAY)							// Blend Overlay
		{
	...
	case (Material::TYPE::AISTANDARDSURFACE):
		...
		case (Material::SPECIAL::NO):
			...
			if (mat.Blend == Material::BLEND::SUBTRACT)							// SUBTRACT
			{
				...
				out << "connectAttr \"" << mat.name << "aiNegate.msg\" \":defaultRenderUtilityList.u\" -na;\n";
			}
