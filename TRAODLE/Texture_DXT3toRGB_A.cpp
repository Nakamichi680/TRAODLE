	...
	for (unsigned int row = 0; row < Ysize; row += 4)
	{
		for (unsigned int column = 0; column < Xsize; column += 4)
		{
			...
			for (unsigned int r = 0; r < 4; r++)
				...
				{
					// Scrittura colore
					unsigned int write_pos = row * Xsize * 3 +  r * Xsize * 3 + column * 3 + c * 3;
					switch (indextable[r][c])
					{
					case 0:
						output_buffer_rgb[write_pos] = c0b;
						output_buffer_rgb[write_pos + 1] = c0g;
						output_buffer_rgb[write_pos + 2] = c0r;
						break;
					case 1:
						output_buffer_rgb[write_pos] = c1b;
						output_buffer_rgb[write_pos + 1] = c1g;
						output_buffer_rgb[write_pos + 2] = c1r;
						break;
					case 2:
						output_buffer_rgb[write_pos] = c2b;
						output_buffer_rgb[write_pos + 1] = c2g;
						output_buffer_rgb[write_pos + 2] = c2r;
						break;
					case 3:
						output_buffer_rgb[write_pos] = c3b;
						output_buffer_rgb[write_pos + 1] = c3g;
						output_buffer_rgb[write_pos + 2] = c3r;
					}

					// Scrittura alpha
					write_pos =  row * Xsize +  r * Xsize + column + c;
					output_buffer_alpha[write_pos] = alphatable[r][c];
				}
		}
	}
}