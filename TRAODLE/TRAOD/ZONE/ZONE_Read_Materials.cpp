...
bool ZONE_Read_Materials (string filename, FBX_EXPORT &FBX, MA_EXPORT &MA)
{
	...
	for (unsigned int m = 0; m < zone_materials_header.nMaterials; m++)
	{
		...
		{
		case (1):
			mat = ZONE_Read_Materials_MayaHw20(zonename, diffuse_transparent, material_name.str(), zone_materials_list);
			break;
		case (2):
			mat = ZONE_Read_Materials_Arnold(zonename, diffuse_transparent, material_name.str(), zone_materials_list);
			break;
		}
