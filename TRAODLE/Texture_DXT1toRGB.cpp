...
void Texture_DXT1toRGB (unsigned int Xsize, unsigned int Ysize, char* input_buffer, char* output_buffer)
{
	for (unsigned int row = 0; row < Ysize; row += 4)
	{
		for (unsigned int column = 0; column < Xsize; column += 4)
		{
			...
			vector < vector <unsigned char> > indextable (4);

			// Lettura tabella colori e tabella indici
			memcpy(&c0, input_buffer + column * 2 + row * Xsize / 2, 2);
			memcpy(&c1, input_buffer + column * 2 + row * Xsize / 2 + 2, 2);
			memcpy(&row1, input_buffer + column * 2 + row * Xsize / 2 + 4, 1);
			memcpy(&row2, input_buffer + column * 2 + row * Xsize / 2 + 5, 1);
			memcpy(&row3, input_buffer + column * 2 + row * Xsize / 2 + 6, 1);
			memcpy(&row4, input_buffer + column * 2 + row * Xsize / 2 + 7, 1);
			...
			// Scrittura pixels nel buffer di output
